...
export default function FarmerDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setStats({ listings: listingsCount || 0, pendingOrders: pendingCount || 0, fulfilledToday: fulfilledCount || 0, earnings: totalEarnings })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F0E8', paddingBottom: '90px' }}>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <p style={{ fontSize: '13px', color: '#888', margin: '0 0 20px' }}>{farmer?.produce_types || 'No produce types set'}</p>
        ...
        {isEmpty ? (
          <div style={{ background: '#2D6A4F', borderRadius: '20px', padding: '28px 20px', textAlign: 'center', marginBottom: '16px' }}>
            ...
            <p style={{ fontSize: '13px', color: '#D8F3DC', margin: '0 0 20px', lineHeight: '1.5' }}>Customers in Lusaka are waiting for fresh produce.</p>
            <button onClick={() => router.push('/farmer/produce/add')} style={{ background: '#fff', border: 'none', borderRadius: '20px', padding: '12px 28px', fontSize: '14px', color: '#2D6A4F', fontWeight: '700', cursor: 'pointer' }}>Add produce</button>
          </div>
        ) : (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
              {[
                ...
                { label: 'Fulfilled today',  value: stats.fulfilledToday },
                { label: 'Total earnings',   value: `K ${stats.earnings.toFixed(2)}`, small: true },
              ].map(s => (
                <div key={s.label} style={{ background: '#fff', borderRadius: '14px', padding: '14px' }}>
                  ...
                  <p style={{ fontSize: s.small ? '16px' : '24px', fontWeight: '700', color: '#2D6A4F', margin: '0' }}>{s.value}</p>
                </div>
              ))}
            </div>
              ...
              {recentOrders.length === 0 ? (
                ...
                recentOrders.map((item, i) => (
                  <div key={item.id} style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: i < recentOrders.length - 1 ? '1px solid #F5F0E8' : 'none' }}>
                    <div>
                      <p style={{ fontSize: '13px', fontWeight: '600', color: '#1a1a1a', margin: '0 0 2px' }}>Order #{item.orders?.id?.slice(-4).toUpperCase()}</p>
                      <p style={{ fontSize: '11px', color: '#888', margin: '0' }}>{item.quantity} {item.produce?.unit} {item.produce?.name}</p>
                    </div>
